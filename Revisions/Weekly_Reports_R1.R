...
# Load Outstanding
  ...
  while(A < 7)
  {
    ...
    if(is.na(sum(Parsing_GK[which(Parsing_GK$Priority == 2 & month(Parsing_GK$Created) == LastSixMonths_GK[A,1] & year(Parsing_GK$Created) == LastSixMonths_GK[A,2]),3])))
    {
      LastSixMonths_GK[A,6] <- 0
    }
    else
    {
      LastSixMonths_GK[A,6] <- round(sum(Parsing_GK[which(Parsing_GK$Priority == 2 & month(Parsing_GK$Created) == LastSixMonths_GK[A,1] & year(Parsing_GK$Created) == LastSixMonths_GK[A,2]),5]) / LastSixMonths_GK[A,5],0)
    }
    
    if(is.na(sum(Parsing_GK[which(Parsing_GK$Priority == 3 & month(Parsing_GK$Created) == LastSixMonths_GK[A,1] & year(Parsing_GK$Created) == LastSixMonths_GK[A,2]),3])))
    {
...
# Change any NaN (divide by 0) to 0
  LastSixMonths_GK <- replace(LastSixMonths_GK,is.na(LastSixMonths_GK),0)
  LastSixMonths_VM <- replace(LastSixMonths_VM,is.na(LastSixMonths_VM),0)
  
# Create Plotting Tables
  LastSix_GK <- data.frame()
  LastSix_VM <- data.frame()
  LastSix_GK <- LastSixMonths_GK
  LastSix_VM <- LastSixMonths_VM
  LastSix_GK$Month <- paste(LastSix_GK$Month," - ",LastSix_GK$Year)
  LastSix_VM$Month <- paste(LastSix_VM$Month," - ",LastSix_VM$Year)
  LastSix_GK <- LastSix_GK[,-2]
  LastSix_VM <- LastSix_VM[,-2]
  
  rm(LastSixMonths_GK,LastSixMonths_VM,Parsing_GK,Parsing_VM)
  
### Plotting All
  # Y Scale - Find Highest # in each of GK and VM Frames
  GK_Scale <- max(LastSix_GK[2:11], na.rm=TRUE)
  VM_Scale <- max(LastSix_VM[2:11], na.rm=TRUE)
  
# Round up to nearest 10
  GK_Scale <- ceiling(GK_Scale/10)*10
  VM_Scale <- ceiling(VM_Scale/10)*10
  
# Melt data to create long vs wide
  df_G <- melt(LastSix_GK,id.vars = "Month")
  df_V <- melt(LastSix_VM,id.vars = "Month")
...
# Plot Parameters
  ...
  TitleGK <- c(paste("Gahcho Kue - SAP Notifications - Last 6 Months (report date:",format(Sys.Date(),"%m/%d/%Y"),")"))
  TitleVM <- c(paste("Victor Mine - SAP Notifications - Last 6 Months (report date:",format(Sys.Date(),"%m/%d/%Y"),")"))
  ...
  ggplot(df_G, aes(x=Month, y=value, fill=variable)) + 
    scale_x_discrete(limits = MonOrderG) +  
  ...
  ggplot(df_V, aes(x=Month, y=value, fill=variable)) + 
    scale_x_discrete(limits = MonOrderV) +  
    geom_bar(stat='identity', position ='dodge', colour = 'black') +
    xlab("Month/Year") + ylab("New Notifications/Average Outstanding") +
    ggtitle(TitleVM) +
    scale_fill_manual(values=cbPalette) +
    labs(fill = "Legend") +
    scale_y_continuous(limits = c(0,VM_Scale), breaks = seq(0,VM_Scale,10), minor_breaks = seq(10,115,20))